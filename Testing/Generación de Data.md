...
<font color = "sky blue">Afiliación de Cliente al Sistema OOBB</font> (Es para afiliar Holding)
- 19 | 1 | 4 | 1 | F6 Crear
- F4 para buscar el cliente
- Completar: F7 Empresas | F8 Cuentas + F10 Trx + F17 Datos Adic
- ------------------
- `Para afiliar clientes al Holding que se creó en el paso anterior`
- 19 | 1 | 4 | 1 |  Buscar por nombre al cliente Principal + Enter
- F7 Empresas
- F7 + F9 Mostrar todo elegir el Cliente Secundario y en la columna S -> "1"
...
- 35: se vincula a un deposito a plazo debe de estar la sección de Captaciones.
...
### <font color = "orange">SESION 05</font>
- Garantía tipo 32
...
- En Proceso, necesita aprobación de todos los datos conforme.
	...
	NO SE PUDO COMPLETAR EL FLUJO DE LEASING PORQUE EL AS400 NO ESTABA OPERATIVO.
### <font color = "orange">SESION 07 parte01</FONT>
- Se resolvió consultas sobre asociación de empresas y usuarios al Holding
- 
### <font color = "orange">SESION 07 parte02</FONT>
...
### <font color = "orange">SESION 09</FONT>
- FACTURAS NEGOCIABLES
- Se deben pasar siempre por refrendo
- PAGARES AL VENCIMIENTO
	- 15 | 4 | 15 Modificar linea de credito
	- Falta completar Renovacion y cancelacion de pagarés
- PAGARES AL VENCIMIENTO CUOTAS
	- WEB COMEX va a revisar Iris
- DMATTO - COMPANIA MINERA CONDESTABLE S.A.

### <font color = "orange">SESION 11</font>
- TEMAS TRATADOS
- Cuentas mancomunadas
- Teoría de transferencias
- Otros Core donde se puede visualizar la información
- Transferencias misma Empresa
- Transferencias mismo Grupo
- **CUANDO SE CREAR UN NUEVO CLIENTE Y ÉSTE SE NECESITA ACCEDER AL OOBB SE DEBE CONFIGURAR SU TARJETA DE COORDENADAS** 

SELECT * FROM ALTEQBSP.SOFBF01 WHERE NOMUSU IN ('ASANCHEZC') 

UPDATE ALTEQBSP.SOFBF01 SET NROTAR = '90000002' WHERE NOMUSU IN ('ATORRESR')

![[Pasted image 20240808185021.png]]

### <font color = "orange">SESION 12</font>
Base de Datos / **Ejecutar scripts de SQL**
IP BD: `Sistema: 180.176.3.136    S21acf3v
Puntos abordados:
- Querys
--<font color = "#FBD25F">TABLA MAESTRO DE CLIENTE  </FONT>
`SELECT *FROM ALTEQBSP.SCLIF01 WHERE DOCUME = '20600508581'

-- <font color = "#FBD25F">TABLA MAESTRO DE CUENTAS</FONT>  
`SELECT * FROM ALTEQBSP.SCTAF21 WHERE DOCU21= '20600508581'

`MARRIE        1  0       TEXT('ESTADO')                                   
`0=ACTIVO                                       
`1=VENCIDO                                      
`3=EN GARANTIA                                  
`9=INMOVILIZADA

-- <font color = "#FBD25F">REGISTRO DE CARTERA</FONT>  
`SELECT * FROM ALTEQBSP.SCARF01 WHERE DOCUME = '20600508581'

-- <font color = "#FBD25F">REGISTRO DE GARANTÍAS </font>
`SELECT * FROM ALTEQBSP.F9201 WHERE DOCUME = '20600508581'

-- <font color = "#FBD25F">REGISTRO DE CARTA FIANZA</font>  
`SELECT * FROM ALTEQBSP.SCFIF01 WHERE DOCCLI = '20600508581'

-- <font color = "#FBD25F">REGISTRO DE LEASING</font>  
`SELECT * FROM ALTEQBSP.SLEAF01 WHERE DOCUME = '20600508581'

-- <font color = "#FBD25F">REGISTRO DE DEPÓSITOS A PLAZO</font>  
`SELECT * FROM ALTEQBSP.SCERF01 WHERE DOCUME = '20600508581'
...
11 AGOSTO Pablo Brito
140 soles

- Anular la Partida Nac de Nelly a Bogota
- Apostilla se tiene que volver a hacer desde Colombia
- Falta la cedula del abuelo y la iglesia donde la bautizaron buscar el acta de bautismo. se corrobora el año de nacimiento
- Ir con un abogado para anular la parti nac nueva 
- Otra cedula colombiana 42 y 43 para que los años concordantes
- Rosanna su PN tiene que ser abogado venezolano y volver a sacar parti nac venez
- Recién se puede presentar documentos


Preguntar por la cartilla y reconocimiento de los Gomez Sanchez esta es la primordial
con esto se obtiene los otros papeles esta la firma de padre y madre
Contiene las fechas de nacimiento de todos.
Echar viajecito para halla... pero donde queda.
Es como las escrituras y ahí esta la fecha de Nelly Luis Barbara
Cedula de todos
Buscar a Gabriel Gomez Gutiérrez
Calle 14 Funeraria Esperanza
